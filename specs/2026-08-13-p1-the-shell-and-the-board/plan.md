...
4.4 Rewrite the `.dark` block. Warm now, switched on in P9. Note the two alpha values — the
...
6.1 Create `src/components/layout/app_sidebar.tsx`. One nav item. The slot comments are the
    contract between this phase and P2/P7/P9:
...
- Build the shell: a collapsible sidebar holding one nav item, **Notes**, plus named slots for
  the New-note (P2), search (P7), and theme (P9) controls. The board fills the rest.
