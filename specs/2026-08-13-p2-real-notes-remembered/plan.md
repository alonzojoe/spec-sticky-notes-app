...
1.4 Write `src/__tests__/reducer_purity.test.ts` for § T19 — a source assertion that
    ...
    `react`. A pure reducer that quietly grows a `Date.now()` in *Markdown and checkboxes* is a test that starts needing
...
```
    ...
    [requirements.md](./requirements.md) § Risks, guarded by an `alert-dialog` in *Polish*. The two
...
is still ***Markdown and checkboxes***. Nothing is scheduled here.
```
...
```md
- Board key: `sticky-notes:board:v1` · Sidebar key: `sticky-notes:sidebar` · Theme key: `sticky-notes:theme` (*Dark mode*)
```
