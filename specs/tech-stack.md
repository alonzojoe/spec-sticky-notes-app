...
| Forms | **`@tanstack/react-form`** | Arrives in P15. **One way to hold what somebody typed, replacing four.** What it replaced: four hand-rolled field containers, two open-and-reset resyncs driven by `useState`, one hand-rolled dirty check, one dismissal path that read values back out of the DOM by CSS selector, and a `useDebounceCallback` pair whose cancel-on-unmount is broken in the library that ships it. Why nothing already here could do the job: `usehooks-ts` has no form primitive and React has no form state, so the alternative was a fifth hand-rolled container written a fifth way. Three packages, one direct dependency; the TanStack cadence is one this project already accepts through the router. **The counter-argument, kept rather than answered:** three of the four forms have one field, and a form library for a single text input is overkill measured on its own — it earns its place on the note view, and on there being one answer for the next form. |
...
- **Every color, radius, shadow and duration comes from a token.** No arbitrary hex values
  ...
  `src/app/main.css` are the only source.
- **File and directory names are `snake_case`**, with the router's own vocabulary allowed on top:
  a leading `_` marks a pathless layout group (`_board`) and `__root.tsx` names the route tree's
  root. `app_sidebar.tsx`, `note_card.tsx`,
...
`@import "tailwindcss";` in `src/app/main.css` — there is no `tailwind.config.js`. Design
...
src/
  app.tsx
  main.tsx
  app/                   // the entry point and the routing              (P11)
    main.tsx             //   builds the router, awaits the first match, mounts it
    main.css             //   @theme tokens, @utility grain, base layer
    routeTree.gen.ts     //   generated by @tanstack/router-plugin — never edited
    config/
      router_config.ts   //   createAppRouter(history) over the generated tree
    routes/
      __root.tsx         //   the document: an <Outlet /> and nothing else
      _board/            //   pathless group: adds no URL segment
        route.tsx        //     AppShell — sidebar, toolbar, providers, shortcuts
        index.tsx        //     /
  ...
  components/
    ...
    board/
      board.tsx          // the cork surface; the grid, and the section it draws
