...
2. **The button.** A shadcn `Button` at the right edge of the existing `app_shell.tsx` header —
   ...
   header becomes the toolbar that P9 and P11 already need (**D2**).
...
- **Delete confirmation.** Still **P12**, and unrelated.
- **Extracting a `top_nav.tsx`.** Considered and rejected for now (**D2**). The header holds two
  controls after this phase. Extract it when P9 or P11 gives it a third.
...
Extracting a `top_nav.tsx` with named slots for P9's search and P11's theme toggle was the
...
P9 puts search and tags in it, P11 puts the theme toggle in it.
